#Доброва Анна
from flask import Flask, render_template, request, send_file  # импортируем Flask и функции для шаблонов, форм и отправки файлов
import pandas as pd  # библиотека pandas для работы с Excel
...
def process():  # функция для обработки загруженного файла
    ...
    output_path = os.path.join(os.path.dirname(__file__), "report.xlsx")  # путь для сохранения отчёта рядом с main.py
